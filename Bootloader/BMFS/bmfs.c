...
int main(int argc, char *argv[]) {
	...
	} else // Opened ok, is it a valid BMFS disk?
	{
		...
		fread(Directory, 4096, 1,
		      disk); // Read 4096 bytes to the Directory buffer
		rewind(disk);
...
void create(char *filename, unsigned long long maxsize) {
	...
	if (findfile(filename, &tempentry, &slot) == 0) {
		...
		for (tint = 0; tint < num_used_entries + 1; tint++) {
			...
			// and the beginning of the current file (or the last data block if
			...
			if (this_file_start - prev_file_end >=
			    blocks_requested) { // fits here
		...
		if (first_free_entry == num_used_entries && num_used_entries + 1 < 64) {
			...
			pEntry =
			    (struct BMFSEntry *)(Directory + (num_used_entries + 1) * 64);
...
void write(char *filename) {
	...
	if (0 == findfile(filename, &tempentry, &slot)) {
		printf("Error: File not found in BMFS. A file entry must first be "
		       "created.\n");
	} else {
